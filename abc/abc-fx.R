...
perturb_particles <- function(dt, cov_mat) {
  ...
  while(dt[, any(!in_prior)]) {
    ...
    for(i in not_in_prior) {
      dt[i, c("lambda_new", "cond_rr_new", "c_new", "s_new", "rr_ai_new", "p_rate_rr_new", "base_male_hiv_incidence_new") := as.list(rmnorm(n = 1, mean = as.vector(c(lambda, cond_rr, c, s, rr_ai, p_rate_rr, base_male_hiv_incidence)), varcov = cov_mat))]
      dt[i, in_prior := is_in_prior(lambda = lambda_new, cond_rr = cond_rr_new, c = c_new, s = s_new, rr_ai = rr_ai_new, p_rate_rr = p_rate_rr_new, base_male_hiv_incidence = base_male_hiv_incidence_new)]
    }
  }
...
is_in_prior <- function(lambda, cond_rr, c, s, rr_ai, p_rate_rr, base_male_hiv_incidence) {
  d <- dbeta(x = lambda, shape1 = priors[param == "lambda", alpha], shape2 = priors[param == "lambda", beta]) *
       ...
       dunif(x = p_rate_rr, min = priors[param == "p_rate_rr", min], max = priors[param == "p_rate_rr", max]) *
       dunif(x = base_male_hiv_incidence, min = priors[param == "base_male_hiv_incidence", min], max = priors[param == "base_male_hiv_incidence", max])
...
calc_weights <- function(prev_iter_particles, weights, curr_iter_particles) {
  ...
  denominator <- calc_denominator_weights(prev_iter_particles = as.matrix(prev_iter_particles), 
                                          weights = weights, 
                                          curr_iter_particles = as.matrix(curr_iter_particles[, .(lambda, cond_rr, c, s, rr_ai, p_rate_rr, base_male_hiv_incidence)]))
