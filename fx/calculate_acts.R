...
calculate_acts <- function(f_dt, m_dt) {
  ...
  dt[, `:=`(n_vi_acts      = pp_vi_acts * n_part,
            ...
            n_acts_adh     = ifelse(adh == 1, (pp_vi_acts + pp_ai_acts) * n_part, 0),
            n_acts_non_adh = ifelse(adh == 0, (pp_vi_acts + pp_ai_acts) * n_part, 0))]
  
  return(dt[, .(n_vi_acts, n_ai_acts, n_acts_adh, n_acts_non_adh)])
}
